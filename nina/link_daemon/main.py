...
def _maybe_boot_ap(coordinator: LinkCoordinator) -> None:
    ...
    if mode == UserMode.FORCE_STA and saved:
        try:
            coordinator.nm.activate_connection(saved[0].uuid)
            coordinator.clear_error()
            log.info("Boot: connected saved profile %s", saved[0].ssid)
            return
        except NMError as e:
            log.warning("Boot STA failed, falling back to AP: %s", e)
            coordinator.set_error(str(e))

    try:
        coordinator.nm.start_hotspot(coordinator.cfg.ap_ssid, coordinator.cfg.ap_password)
        coordinator.ps.ap_started = True
        coordinator.store.save(coordinator.ps)
        log.info("Boot AP started SSID=%s", coordinator.cfg.ap_ssid)
    except NMError as e:
        coordinator.set_error(str(e))
...
def main() -> None:
    ...
    cfg = load_config()
    nm: NMBackend = mock_backend() if cfg.mock_nm else NMBackend(mock=False)
