...
| `NINA_GOTO_FWD_CLEAR_MM` | 700 | Min forward free space (mm) to commit to driving along the planned path. Wander uses `NINA_AUTO_FWD_CLEAR_MM` (1200). **Note:** the goto pilot still applies `NINA_AUTO_ESTOP_MM` (default **850**) first—forward readings **below** that trigger immediate backoff, so with stock estop the **effective** “must be clear to drive” floor is **850 mm** unless you lower `NINA_AUTO_ESTOP_MM` (e.g. 600) for tighter indoor runs. |
