...
class MotionPlayerWrapper:
    def __init__(self, ser: serial.Serial):
        ...
        # Asynchronous state observer thread
        self._fb_stop = threading.Event()
        # Set this event to pause bus polling during playback — prevents the
        # feedback thread from fighting play_frames for the serial bus.
        self._fb_pause = threading.Event()
        self._fb_thread = threading.Thread(target=self._feedback_loop, daemon=True)
        self._fb_thread.start()
    ...
    def servo_move(self, servo_id: int, value: int, speed: int = 360, mode: Optional[str] = None) -> Optional[int]:
        """
        Move a single servo.
        - mode="abs": value is raw goal position.
        - mode="rel": value is a signed offset from current.
    ...
    def multi_servo_move(self, servo_dict: Dict[str, int], speed: int = 360, mode: Optional[str] = None):
        """
        Move multiple servos at once while holding a single lock.
        Supports both Abs (1-5) and Rel (6-7) IDs in the same dictionary.
        servo_dict: {servo_id: goal_val_or_offset}
    ...
    def _feedback_loop(self):
        ...
        logging.info("Starting background feedback observer...")
        while not self._fb_stop.is_set():
            # Yield the bus entirely while play_frames is running.
            if self._fb_pause.is_set():
                time.sleep(0.05)
                continue
            try:
                # We poll incrementally to minimize bus occupancy per lock
                for sid in SERVO_IDS:
                    if self._fb_stop.is_set() or self._fb_pause.is_set():
                        break
                    with self.lock:
                        if self.ser and self.ser.is_open:
                            pos = read_reg(self.ser, sid, *REG_PRESENT_POS)
    ...
    def _play_loop(self, frames, loop):
        # Pause the background feedback poller for the entire duration of
        # playback.  play_frames / loop_frames do their own bus reads (S6, S7
        # present position) and the feedback thread racing those reads is what
        # causes "device reports readiness to read but returned no data".
        self._fb_pause.set()
        try:
            if self.ser and self.ser.is_open:
                for sid in SERVO_IDS:
            ...
            self.is_playing = False
            self._fb_pause.clear()   # resume background polling
            self._stop_flag.clear()
    ...
    def process_message(self, client_sock, line):
        ...
        def _normalize_frame(frame_dict: dict) -> dict:
            ...
            normalized = {
                "delay": float(frame_dict.get("delay", 0.5)),
                "duration": float(frame_dict.get("duration", 1.0)),
                "speed": int(frame_dict.get("speed", 360)),
                "servos": servos_out,
            }
            if "actuator" in frame_dict:
                if not isinstance(frame_dict["actuator"], dict):
